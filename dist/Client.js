...
const fsReadDir = util_1.promisify(browserify_fs_1.readdir);
const fsMkDir = util_1.promisify(browserify_fs_1.mkdir);
const fsStat = util_1.promisify(browserify_fs_1.stat);
const fsOpen = util_1.promisify(browserify_fs_1.open);
const fsClose = util_1.promisify(browserify_fs_1.close);
const fsUnlink = util_1.promisify(browserify_fs_1.unlink);
/**
...
class Client {
    ...
    async _uploadLocalFile(localPath, remotePath, command, options) {
        ...
        const source = browserify_fs_1.createReadStream("", {
            fd,
    ...
    async _downloadToFile(localPath, remotePath, startAt) {
        ...
        const destination = browserify_fs_1.createWriteStream("", {
            fd,
            start: startAt,
            autoClose: false
        });
